  ...
  if (args) {
    ...
    Object.keys(args).forEach(key => {
      ...
      if (inType === 'body') {
        ...
        if (typeName.startsWith && typeName.startsWith('[') && typeName.endsWith(']')) {
          typeName = typeName.replace('[', '').replace(']', '')

          let tcType = guessType(typeName)

          if (tcType && tcType._gqcFields) {
            const schema = generateTypeObject(tcType, schemas)
  ...
  if (fields) {
    Object.keys(fields).map(key => {
      ...
      if (type) {
        ...
        } else if (ofType && ofType._gqType && ofType._gqType.name === 'String') {
          setPropertyType(ofType._gqType.name, key, true)
        } else if (ofType && ofType._gqType && ofType._gqType.name === 'Number') {
          setPropertyType(ofType._gqType.name, key, true)
        } else if (ofType && ofType._gqType && ofType._gqType.name === 'Float') {
          setPropertyType(ofType._gqType.name, key, true)
        } else if (ofType && ofType._gqType && ofType._gqType.name === 'Boolean') {
          setPropertyType(ofType._gqType.name, key, true)
        } else if (ofType && ofType._gqType && ofType._gqType.name === 'Int') {
          setPropertyType(ofType._gqType.name, key, true)
        } else if (type.name) {
