...
const filterMiddlewaresByName = name => m => {
  try {
    ...
    console.log('filterMiddlewaresByName ', err)
    throw new Error(err)
  }
}
...
const loadResolverHook = (hook, resolver, isPolicy) => {
  ...
  if (typeof hook === 'string') {
    ...
    const rPath =
      isPolicy
        ? `${cwd}/src/policies/${controller}Policy`
        : `${c._resolversPath}/${controller}Resolver`
    if (action) {
...
const loadResolverFunc = resolver => {
  ...
  if (typeof c._resolve === 'string') {
    ...
    if (action) {
      const CtrlClass = require(`${c._resolversPath}/${controller}Resolver`)
      const Ctrl = new CtrlClass()
      resolverFunc = Ctrl[action]
    } else {
      resolverFunc = require(`${c._resolversPath}/${controller}Resolver`)
    }
  } else {
...
const prepareResolveFunc =
  (func, resolvesPath, isPolicy) => {
    try {
      ...
      if (typeof resolverFunc !== 'function') {
        throw new TypeError('Resolve is not a valid function', resolvesPath)
      }
