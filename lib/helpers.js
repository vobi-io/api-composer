...
const guessType = type => {
  if (type) {
    ...
    switch (type.constructor) {
      ...
      case Array:
        const typeConst = type[0].constructor
        if (typeConst === Object) {
          const gqType = createType(type[0])._gqType
          return arrayTypeToString([gqType])
        } else if (typeConst === String) {
          return arrayTypeToString(type)
        } else if (typeConst === ObjectTypeComposer) {
          const gqType = type[0]._gqType
          return arrayTypeToString([gqType])
        }
      default:
        break
  ...
  (req, resolver) => {
    ...
    if(_args){
      Object.keys(_args).forEach(key => {
        const arg = _args[key]
        let value = req[arg.in || inValue][key]
        ...
        if (value !== undefined) {
          args[key] = parseValue(arg.type || arg, value)
        }
      })
    }
