...
class API extends Macroable {
  ...
  add({ name, methods, args, path, type, kind = 'query', resolve = async () => {} }) {
    ...
    if (this._defaultFieldsSelection) {
      resolver.restDefaultFieldsSelection(this._defaultFieldsSelection)
    }
    if (this._restFieldsSelectionDepthLimit) {
      resolver.restFieldsSelectionDepthLimit(this._restFieldsSelectionDepthLimit)
    }
    resolver.graphqlOnly(this._isGraphqlOnly)
    ...
    if (this._policiesPath) {
      resolver.policiesPath(this._policiesPath)
    }
    if (resolve) {
